...
### You should have 41 observations/studies
```
...
boxplot(yi ~ measure_f, data = depression_symptoms, xlab = "Depression Measure", 
        ylab = "SMD Effect Size", col=c("green","pink", "orange", "blue"))

```

There are only three studies that measured depression using the BDI, so we would want 
to interpret any results from analyses containing this moderator with caution. 
...
is the inclusion of - 1 following the moderator variable. Additionally, the Test of Moderators 
in the prior specification (above) is the appropriate omnibus test for the relationship between 
included moderators and an overall effect size estimate.
...
# Are depression prevention programs delivered in schools more effective
when they are delivered as primary prevention?
...
### Does the effect of depression prevention programs on depression symptoms vary 
depending on how depression symptoms were measured, when controlling for baseline depression?
```{r multi meta-reg}
...
### if time permits, get into groups of two and work together to run one last moderation 
analysis using meta-regression. Your group can choose to use one of the following: 
